...
function loadMeshOpt(vertexCount, attrType, attributeDim, oriBuffer, vertexPackage, compressOptions) {
  ...
  switch (attrType) {
    ...
    case AttributeType.SecondTexcoord:
      ...
      if (
        (compressOptions & 0x10) === 0 &&
        (attrType === AttributeType.FirstTexcoord || attrType === AttributeType.SecondTexcoord)
      ) {
...
function parseMeshOpIndexPackage(buffer, view, bytesOffset, arrIndexPackage, version) {
  ...
  for (let k = 0; k < indexPackageCount; k++) {
    ...
    const indexBufferInt32 = new Uint32Array(
      decodeIndices.buffer,
      decodeIndices.byteOffset,
      decodeIndices.byteLength / 4
    );
...
function parseSkeleton(buffer, view, bytesOffset, geoPackage, version) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (version === 3) {
      switch (tag) {
        case S3MBVertexTagV3.Standard:
          tag = S3MBVertexTag.SV_Standard;
          break;
        case S3MBVertexTagV3.Draco:
          tag = S3MBVertexTag.SV_DracoCompressed;
          break;
        case S3MBVertexTagV3.MeshOpt:
          tag = S3MBVertexTag.SV_Compressed;
          break;
        default:
          break;
      }
